...
from vyos.vpp.config_resource_checks.cpu import available_core_count
...
def available_memory(config: dict) -> int:
    ...
    if config.get('effective'):
        ...
        memory_effective = total_memory_required(config['effective'])
        memory_used = memory_used - memory_effective

    return memory_total - memory_used


def buffer_size(settings: dict, cpus_count: int) -> int:
    buffers_per_numa = int(
        settings.get('buffers', {}).get('buffers_per_numa', constants.BUFFERS_PER_NUMA)
    )
    data_size = int(settings.get('buffers', {}).get('data_size', constants.DATA_SIZE))
    buffers_memory = buffers_per_numa * data_size * cpus_count
    return buffers_memory


def hugepage_size(settings: dict) -> int:
    nr_hugepages = settings.get('host_resources', {}).get('nr_hugepages')
    if nr_hugepages:
        return int(nr_hugepages) * 2 * 1024**2
    else:
        return human_page_memory_to_bytes(constants.HUGEPAGE_SIZE)


def netlink_buffer_size(settings: dict) -> int:
    return int(
        settings.get('lcp', {}).get('rx_buffer_size', constants.NETLINK_RX_BUFFER_SIZE)
    )


def main_heap_page_size(settings: dict) -> int:
    heap_page_size = settings.get('memory', {}).get(
        'main_heap_page_size', constants.MAIN_HEAP_PAGE_SIZE
    )
...
def memory_main_heap(settings: dict) -> int:
    ...
    if not heap_size:
        heap_size = constants.MAIN_HEAP_SIZE
...
def ipv6_heap_size(settings: dict) -> int:
    ...
    if not heap_size:
        heap_size = constants.IPV6_HEAP_SIZE
    return human_memory_to_bytes(heap_size)
...
def statseg_size(settings: dict) -> int:
    statseg_memory = settings.get('statseg', {}).get(
        'size', constants.STATSEG_HEAP_SIZE
    )
    return human_memory_to_bytes(statseg_memory)


def statset_page_size(settings: dict) -> int:
...
def total_memory_required(settings: dict) -> int:
    mem_required = hugepage_size(settings)
    mem_stats = {
        'memory_buffers': buffer_size(settings, available_core_count()),
        'netlink_buffer_size': netlink_buffer_size(settings),
        ...
        'statseg_size': total_statseg_size(
            ...
            _statseg_page=statset_page_size(settings),
        ),
